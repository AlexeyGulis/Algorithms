...
class Solution:
    ...
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        h = len(heights)
        w = len(heights[0])
        double_heights = [[0 for x in range(w)] for y in range(h)]
        for i in range(len(heights)):
            for j in range(len(heights[i])):
                self.get_pacific = False
                self.get_atlantic = False
                self.help_rec(heights, double_heights, i, j, [])
                if self.get_atlantic and self.get_pacific:
                    double_heights[i][j] = 1
                    answer.append([i, j])
        return answer

    def help_rec(self, heights, double_heights, i, j, way):
        if [i, j] in way:
            return
        way.append([i, j])
        if self.get_atlantic and self.get_pacific:
            return
        if double_heights[i][j] == 1:
            self.get_atlantic = True
            self.get_pacific = True
        if i == 0 or j == 0:
            self.get_pacific = True
        if i == len(heights) - 1 or j == len(heights[i]) - 1:
            self.get_atlantic = True
        if j + 1 < len(heights[i]) and heights[i][j + 1] <= heights[i][j]:
            self.help_rec(heights, double_heights, i, j + 1, way)

        if j - 1 >= 0 and heights[i][j - 1] <= heights[i][j]:
            self.help_rec(heights, double_heights, i, j - 1, way)

        if i + 1 < len(heights) and heights[i + 1][j] <= heights[i][j]:
            self.help_rec(heights, double_heights, i + 1, j, way)
